...
#    Then verify dashboard data for tournament for "2" teams.
  ...
  @Scoreboard_SCOR60
  Scenario: Scenario_08: Add tournament and verify the added tournament for past date.
    Given User log in to "scoreboard" for "manager" UI and present at the home page.
    Then user click on "tournaments module" button.
...
#      | Men    |
#    Then user click on "players module" button.
#    Then user will add "25" players as "old" for "2" team with following details and verify them.
#      | Player Image | Player DOB | Country | State     | City   | Gender | Role    | Skills |
#      | player.jpg   | 01-01-1997 | India   | Rajasthan | Jaipur | Men    | Batsman | 3      |
#    Then user click on "venue module" button.
#    Then user will "add" the "Venue" with count of "1" with following details and verify.
...
#    Then user click on "Save Selection" button.
#    Then verify the validation message "Each Team Must Have A Minimum Of 11 And A Maximum Of 25 Active Players.".
#    Then user add "25" players for team1 and "25" players for team2 and verify.

  @Scoreboard_SCOR-426
  Scenario: Scenario_10: Date range filter not working on Tournament module
    Given User already log in to "scoreboard" for "manager" UI and present at the home page.
    Then user click on "Tournaments module" button.
  ...
  @Scoreboard_SCOR76
  Scenario: Scenario_11: verify the tournament dates descending order
    Given User log in to "scoreboard" for "manager" UI and present at the home page.
