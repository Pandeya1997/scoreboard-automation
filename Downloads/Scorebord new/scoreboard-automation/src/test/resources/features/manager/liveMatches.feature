...
Feature: Scoreboard --> live match page
  Here we will test the functionality of live match page tab on manager UI.
...
#    Then user verify the login "success" message "Sign-In Success".
  ...
  @Scoreboard_NonLiveMatchVerification
  Scenario: Scenario_03: live match verification on the agent UI
...
#    Then user verify the login "success" message "Sign-In Success".
  ...
  @Scoreboard_SCOR416
  Scenario: Scenario_05: Clear button not clickable on the Live Match filter when filtering by venue.
    Given User already log in to "scoreboard" for "manager" UI and present at the home page.
    Then user click on "live matches module" button.
