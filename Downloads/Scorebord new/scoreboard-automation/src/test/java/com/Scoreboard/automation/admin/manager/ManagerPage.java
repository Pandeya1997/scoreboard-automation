...
public class ManagerPage extends CommonMethods {
    ...
    @FindBy(xpath = "//div[@class='rit-pagi-bx']")
    WebElement paginator;

    @FindBy(xpath = "(//input[@name='mobile_no']//following::div)[1]")
    WebElement managerMobileNoError;
...
//            }
    ...
    public void managerMobileNoValidation() {
        try {
            managerName = "AutomationManager" + getRandomString();
            mobileNo = generateUnique10DigitNumber();
            ...
            baseUtil.explicitWait(PathConstants.WAIT_LOW);
            baseUtil.enterText(managerNameTextbox, managerName);
            baseUtil.enterText(managerMobileNoTextbox, "");
            baseUtil.enterText(managerPwdTextbox, "P@ssword1!");
            baseUtil.click(managerSubmitFormBtn);
            logger.info("Verifying validation without mobile number...");
            Assert.assertEquals(getTestBase().getReadResources().
                    getDataFromPropertyFile("Global","no_number"),managerMobileNoError.getText());

            baseUtil.enterText(managerMobileNoTextbox, "");
            baseUtil.click(managerSubmitFormBtn);
            logger.info("Verifying validation less then 10 digit mobile number...");
            Assert.assertEquals(getTestBase().getReadResources().
                    getDataFromPropertyFile("Global","no_number"),managerMobileNoError.getText());
        } catch (Exception e) {
            logger.error("Unable to add manager due to: ", e);
    ...
    public void verifyPaginator(String page){
        logger.info("verifying paginator on the "+page);
        try{
            Assert.assertTrue(baseUtil.isElementDisplayed(paginator));
        }catch (Exception e){
            Assert.fail("unable to verify the paginator due to "+e.getMessage());
        }
    }
}
